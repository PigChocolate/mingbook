package ming.book.action;

import com.alibaba.fastjson.JSON;
import ming.book.bean.Member;
import ming.book.bean.Record;
import ming.book.bean.User;
import ming.book.biz.MemberBiz;
import ming.book.biz.RecordBiz;
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch(type){
            case "add":
               ...
               if(count>0){
                   out.println("<script>alert('图书借阅成功');location.href='rent_list.jsp';</script>");
               }else{
                   out.println("<script>alert('图书借阅失败');location.href='rent_list.jsp';</script>");
               }
                break;
            case "queryback":
                 //1.获取会员的身份证号
            ...
            case "back":
                ...
                //5.归还
                if(count2>0){
                    out.println("<script>alert('归还成功');location.href='rent_list.jsp';</script>");
                }else{
                    out.println("<script>alert('归还失败');location.href='rent_list.jsp';</script>");
                }
                break;
            case "keep":
                ...
                if(count3>0){
                    out.println("<script>alert('续借成功');location.href='rent_list.jsp';</script>");
                }else{
                    out.println("<script>alert('续借失败');location.href='rent_list.jsp';</script>");
                }
                break;
            case "doajax":
